...
- 新增左侧菜单 `T+1交易`，也可直接访问 `/t1-trading`，用于 A 股 T+1 规则下的模拟交易和 T+1 候选分析观察。
- 项目整体 UI 已统一为金融行业工作台风格：深色交易侧栏、中性低噪声背景、紧凑表格、红绿盈亏色、等宽数字和更清晰的卡片分层。
- 后端接口：
  - `POST /api/t1-analysis/run`：手动运行一次 T+1 分析，结果继续写入现有 T+1 分析结果表。
  ...
  - `GET /api/t1-trading/task`：查看 T+1 到期执行任务状态。
  - `POST /api/t1-trading/task/start`：启动或更新 T+1 到期执行任务，按间隔自动读取实时行情并模拟交易。
  - `POST /api/t1-trading/task/stop`：停止 T+1 到期执行任务。
...
- T+1 到期执行任务运行在后端 FastAPI 进程内，可在页面中启动、停止和查看下次到期时间；任务只做模拟交易，不连接真实券商，也不构成投资建议。
